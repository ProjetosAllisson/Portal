...
public class DocumentoViewDAO extends GenericDAO<DocumentoView> {
	...
	public List<DocumentoView> filtrados(FiltroDocumento filtro) {
		...
		} else if (!filtro.isPesquisaAdmin()) {
	...
	private Criteria criarCriteriaParaFiltro(FiltroDocumento filtro) {
		...
		if (this.usuario.getCliente() != null) {
			...
			if (this.usuario.consultaPorGrupoCliente()) {
				remetente = Restrictions.eq("grupo_remetente", usuario
						.getCliente().getGrupoCliente().getGrupo());
				destinatario = Restrictions.eq("grupo_destinatario", usuario
						.getCliente().getGrupoCliente().getGrupo());
			} else {
				remetente = Restrictions.eq("cgc_remetente", usuario
						.getCliente().getCgc());
				destinatario = Restrictions.eq("cgc_destinatario", usuario
						.getCliente().getCgc());
				consignatario = Restrictions.eq("cgc_pagador", usuario
						.getCliente().getCgc());
		...
		if (filtro.getDataInicio() == null
				...
				&& (filtro.getNota_fiscal() == null || filtro.getNota_fiscal()
						.equals("")
		...
		if (filtro.getNota_fiscal() != null
			...
			//----esta com like, porque na view esta com a list, e se tiver mais de uma nota só acha com like.
